...
public class AdaptivePurePursuitController {
    ...
    private CountingDelay timeout = new CountingDelay();
    ...
    public PathPlannerTrajectory setNewTrajectory(
        ...
        currentTrajectory = trajectoryHandler.setTrajectory(pathName, isReversed, maxVelocity, maxAcceleration);

        // Store states in ArrayList
        trajectoryStates = trajectoryHandler.getStatesFromTrajectory();

        PathPlannerTrajectory.PathPlannerState endState = trajectoryStates.get(trajectoryStates.size() - 1);
        SmartDashboard.putNumberArray("Auto/final pose before extrapolation", new Double[]{endState.poseMeters.getX(), endState.poseMeters.getY(), endState.holonomicRotation.getDegrees()});

        trajectoryStates = trajectoryHandler.extrapolateStates(trajectoryStates, PATH_END_DISTANCE);
    ...
    public Pose2d getTargetPose2d(Pose2d currentPose) {
        ...
        SmartDashboard.putNumber("Auto/closest index point", closestPosIndex);
...
//        Translation2d finalPosition = currentTrajectory.getEndState().poseMeters.getTranslation();
        Translation2d finalPosition = trajectoryStates.get(trajectoryStates.size() - 1).poseMeters.getTranslation();

        SmartDashboard.putNumberArray("Auto/final pose (no rotation)", new Double[]{finalPosition.getX(), finalPosition.getY(), 0.0});

        // Check if path is finished
        double targetDistance = finalPosition.getDistance(currentPose.getTranslation());

        SmartDashboard.putNumber("Auto/target distance", targetDistance);
...
//            if (timeout.delay(1)) {
//                DataLogManager.log("Finish path, going too slow");
//                pathFinished = true;
//            }
//        } else {
//            timeout.reset();
//        }
    ...
    /**
     * Gets the current {@code ArrayList<PathPlannerTrajectory.PathPlannerState>} that the robot will follow
     *
     * @return {@code ArrayList<PathPlannerTrajectory.PathPlannerState>} that the robot will follow
     */
    public ArrayList<PathPlannerTrajectory.PathPlannerState> getTrajectoryStates() {
    ...
    public double getTargetVelocityForCurrentPose(Pose2d currentPose) {
        ...
        // TODO change to pathplanner state array
